...
var progressButton = {
    loader: function(form) {
        var $form = form,
            progressBar = $form.find(".ss-progress-button .ss-progress-inner"),
            buttonText = $form.find(".ss-progress-button .ss-progress-button-content"),
    ...
    success: function(form) {
        var $form = form,
            buttonText = $form.find(".ss-button .ss-progress-button-content, .ss-contact-button .ss-progress-button-content"),
            successIcon = $form.find(".ss-progress-button .state-success"),
    ...
    error: function(form) {
        var $form = form,
            buttonText = $form.find(".ss-button .ss-progress-button-content, .ss-contact-button .ss-progress-button-content"),
            errorIcon = $form.find(".ss-progress-button .state-error"),
        ...
        if ($.tools.validator != undefined) {
            $.tools.validator.addEffect("contact_form", function(errors) {
                $.each(errors, function(index, error) {
                    ...
                    input.addClass('ss-invalid');
                });
            }, function(inputs) {
                inputs.removeClass('ss-invalid');
            });
            ...
            $(".captcha-form").each(function() {
              $(this).on("focus", function() {
                $(this).attr("placeholder", ss_captcha_placeholder).removeClass('contact-captcha-invalid contact-captcha-valid');
              });
            });

            var changeCaptcha = function() {
                $(".captcha-image").attr("src", ss_theme_dir + "/captcha/captcha.php?" + Math.random());
            }

            var sendForm;
            var checkCaptcha = function(form, enteredCaptcha) {
                $.get(ss_theme_dir + "/captcha/captcha-check.php", {
                    captcha: enteredCaptcha
                }).done(function(data) {
                    if (data != "ok") {
                        changeCaptcha();
                        form.find(".captcha-form").val("").addClass('contact-captcha-invalid').attr("placeholder",ss_captcha_invalid_txt);
                    } else {
                        sendForm();
                        changeCaptcha();
                        form.find(".captcha-form").val("").addClass('contact-captcha-valid').attr("placeholder", ss_captcha_correct_txt);
                    }
                });
            }

            $('.ss-contact-form').validator({
                effect: 'contact_form'
            }).submit(function(e) {
                var form = $(this);
                if (!e.isDefaultPrevented()) {

                	if (form.parent().hasClass('ss-light-style')) {
                		$form_action = 'st_contact_form_light';
                	}else{
                		$form_action = 'st_contact_form_dark';
                	}
                    ...
                    sendForm = function() {
                        ...
                        $.post(ajaxurl, data, function(response) {
                            form.find('.ss-contact-loading').fadeOut('slow');
                            form.find('input').val("");
